...
standalone-diagnose: standalone-ssh-test
	@echo '== 一键诊断 $(PKG_NAME) =='
	@$(SSH) $(SSH_OPTS) $(SSH_HOST) 'echo "[1] 固件信息"; grep -E "DISTRIB_(ID|RELEASE|DESCRIPTION)" /etc/openwrt_release 2>/dev/null; echo; echo "[2] rpcd 进程"; pgrep -af rpcd || echo "rpcd 未运行"; echo; echo "[3] 插件文件"; ls -la /usr/libexec/rpcd/ 2>/dev/null | grep -E "mtk|map" || echo "未找到插件文件"; echo; echo "[4] ubus 对象"; ubus list 2>&1 | grep -E "mtk|map" || echo "(mtk-dash2 未注册 -> Object not found 的根源)"; echo; echo "[5] 手动执行 list（注册用）"; timeout 5 /usr/libexec/rpcd/mtk-dash2 list 2>&1 | head -c 200; echo; echo "[6] 手动执行 call status"; timeout 5 sh -c '"'"'echo "{}" | /usr/libexec/rpcd/mtk-dash2 call status'"'"' 2>&1 | head -c 400; echo; echo "[7] Lua 依赖"; lua -e "require(\"luci.jsonc\"); print(\"luci.jsonc OK\")" 2>&1 | head -3; lua -e "require(\"nixio.fs\"); print(\"nixio.fs OK\")" 2>&1 | head -3; echo; echo "[8] ACL"; ls -l /usr/share/rpcd/acl.d/ 2>/dev/null | grep -E "mtk|map" || echo "未找到 ACL"; echo; echo "[9] /tmp 空间与 LuCI 缓存"; df -h /tmp | tail -1; ls -d /tmp/luci-* 2>/dev/null || echo "(缓存不存在，LuCI 将重建)"; echo; echo "[10] 相关日志"; logread 2>/dev/null | grep -iE "rpcd|mtk-dash|map-easymesh" | tail -15'
